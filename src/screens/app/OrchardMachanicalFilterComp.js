...
export default function OrchardMachanicalFilterComp({
  applyFilter = [],
  onCLose = () => {},
  userData,
  ...
  return (
    <View>
      <DropDown
        ...
        value={selectedDprType?.name}
        selectItem={(item) => {
          setselectedDprType(item);
          if (item?.name == "Orchard") {
            setselectedNursery("");
            setselectedPlan("");
          } else {
            setselectedOrchard("");
